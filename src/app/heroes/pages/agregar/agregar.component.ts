...
export class AgregarComponent {
  ...
  guardar() {
    ...
    if (this.heroe.id) {
      //Si tiene un id, actualizamos (petición PUT)
      this.heroesService.actualizarHeroe(this.heroe)
        .subscribe(heroe => {
          this.mostrarSnackBar('Has actualizado correctamente!');
          this.heroe = heroe;
        })
    } else {
      // Si no tiene un id, el heroe no existe y lo agregamos (petición POST )
      this.heroesService.agregarHeroe(this.heroe)
        .subscribe(heroe => {
          this.router.navigate(['/heroes/editar', heroe.id]);
          this.mostrarSnackBar('Has agregado un heroe correctamente!');
        })
    }
  }
